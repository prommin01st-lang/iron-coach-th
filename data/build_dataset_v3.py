"""
สร้าง dataset v3 = หมวดเดิมทั้งหมด (cat1-7) + refusal เพิ่ม (cat8)
ความต่างจาก v2:
...
for f in sorted(glob.glob(os.path.join(SCRATCH, "cat*.json"))):
    ...
    for ex in data:
        ...
        if not all(isinstance(m["content"], str) and m["content"].strip() for m in clean):
            dropped += 1; continue
        clean[0]["content"] = SYS  # normalize เป็น system prompt v3
...
with open(os.path.join(HERE, "train_v3.jsonl"), "w", encoding="utf-8") as fh:
    for ex in train:
        fh.write(json.dumps(ex, ensure_ascii=False) + "\n")
with open(os.path.join(HERE, "val_v3.jsonl"), "w", encoding="utf-8") as fh:
    for ex in val:
        fh.write(json.dumps(ex, ensure_ascii=False) + "\n")
